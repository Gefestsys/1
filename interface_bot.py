...
from config import TELEGRAM_TOKEN, MAX_RETRIES, RETRY_DELAY, logger
from db_sqlite import UserManager, init_db
from localization import get_text

from activation_manager import ActivationCodeManager
...
async def handle_activation_code(message: Message, state: FSMContext):
    ...
    # Используем новую систему активационных кодов
    if ActivationCodeManager.is_valid_activation_code(code):
...
@dp.message(Command("admin_add_activation"))
@error_handler
async def handle_admin_add_activation(message: Message):
    """Добавление нового кода активации"""
    user_id = message.chat.id
    ...
    try:
        ...
        if len(args) < 1:
            await message.answer(
                "Использование: /admin_add_activation <код>\n\n"
                "Пример: /admin_add_activation NEWCODE123"
            )
            return

        activation_code = args[0].strip()

        if len(activation_code) < 3:
            await message.answer("❌ Код активации должен содержать минимум 3 символа")
            return

        success = ActivationCodeManager.add_activation_code(activation_code, f"admin_{user_id}")

        if success:
            await message.answer(
                f"✅ Код активации добавлен!\n\n"
                f"🔑 Код: {activation_code}\n"
                f"👤 Создан администратором: {user_id}",
            )
            logger.info(f"Admin {user_id} added activation code: {activation_code}")
        else:
            await message.answer(f"❌ Код активации {activation_code} уже существует")

    except Exception as e:
        logger.error(f"Error adding activation code by admin {user_id}: {e}")
        await message.answer("❌ Произошла ошибка при добавлении кода активации")


@dp.message(Command("admin_remove_activation"))
@error_handler
async def handle_admin_remove_activation(message: Message):
    ...
    try:
        ...
        if len(args) < 1:
            await message.answer(
                "Использование: /admin_remove_activation <код>\n\n"
                "Пример: /admin_remove_activation OLDCODE123\n\n"
                "⚠️ Постоянные коды (например, 'Spike') нельзя удалить"
            )
            return

        activation_code = args[0].strip()
        success = ActivationCodeManager.remove_activation_code(activation_code)

        if success:
            await message.answer(
                f"✅ Код активации удален!\n\n"
                f"🔑 Код: {activation_code}",
            )
            logger.info(f"Admin {user_id} removed activation code: {activation_code}")
        else:
            await message.answer(
                f"❌ Не удалось удалить код активации {activation_code}\n\n"
                f"Возможные причины:\n"
                f"• Код не найден\n"
                f"• Код является постоянным (нельзя удалить)",
            )

    except Exception as e:
        logger.error(f"Error removing activation code by admin {user_id}: {e}")
        await message.answer("❌ Произошла ошибка при удалении кода активации")


@dp.message(Command("admin_list_activation"))
@error_handler
async def handle_admin_list_activation(message: Message):
    """Список всех кодов активации"""
    user_id = message.chat.id
    ...
    try:
        activation_codes = ActivationCodeManager.get_all_activation_codes()

        if not activation_codes:
            await message.answer("📝 Коды активации не найдены")
            return

        response = f"🔑 Список кодов активации\n\n"

        for code_info in activation_codes:
            status = "🔒 Постоянный" if code_info['is_permanent'] else "📝 Временный"
            response += (
                f"{code_info['code']} - {status}\n"
                f"👤 Создал: {code_info['created_by_admin']}\n"
                f"📅 Создан: {code_info['created_at']}\n\n"
            )

        # Разбиваем длинные сообщения
        if len(response) > 4000:
            messages = []
            current_message = "🔑 Список кодов активации\n\n"
            ...
            for code_info in activation_codes:
                ...
                if len(current_message + code_text) > 4000:
                    messages.append(current_message)
                    current_message = code_text
                else:
                    current_message += code_text
        ...
        logger.error(f"Error listing activation codes by admin {user_id}: {e}")
        await message.answer("❌ Ошибка при получении списка кодов активации")


@dp.message(Command("admin_activation_stats"))
@error_handler
async def handle_admin_activation_stats(message: Message):
    """Статистика по кодам активации"""
    user_id = message.chat.id
    ...
    try:
        stats = ActivationCodeManager.get_activation_stats()

        response = (
            f"📊 Статистика кодов активации\n\n"
            f"🔑 Всего кодов: {stats['total']}\n"
            f"🔒 Постоянных: {stats['permanent']}\n"
            f"📝 Временных: {stats['temporary']}\n"
        )

        await message.answer(response)

    except Exception as e:
        logger.error(f"Error getting activation stats by admin {user_id}: {e}")
        await message.answer("❌ Ошибка при получении статистики")
...
<b>Коды активации:</b>
• /admin_add_activation &lt;код&gt; - Добавить код активации
• /admin_remove_activation &lt;код&gt; - Удалить код активации
• /admin_list_activation - Список всех кодов
• /admin_activation_stats - Статистика кодов

<b>Промокоды:</b>
• /admin_create_promo &lt;дни&gt; [описание] [срок_дней] - Создать промокод
• /admin_bulk_promo &lt;кол&gt; &lt;дни&gt; [описание] [срок_дней] - Создать несколько промокодов
• /admin_promo_stats - Статистика промокодов
• /admin_promo_list [страница] - Список промокодов
• /admin_promo_info &lt;код&gt; - Информация о промокоде
• /admin_delete_promo &lt;код&gt; - Удалить промокод
• /admin_cleanup_promo - Очистить истекшие промокоды
