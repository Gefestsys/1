...
#define FLARE_AMOUNT 3.0
#define FLARE_EXP 1.8
#define TOP_FADE_START 0.02
#define TOP_FADE_EXP 0.9
...
#define W_CURVE_AMOUNT 10.0
#define W_CURVE_RANGE (FLARE_HEIGHT - 3.0)
#define W_BOTTOM_EXP 14.0
...
void mainImage(out vec4 fc,in vec2 frag){
    ...
    for(int k=-TAP_RADIUS;k<=TAP_RADIUS;++k){
        ...
        vec2 sig=vec2(wx*uCoreScale,1.0),p=vec2(0.0,yp);
...
export const LaserFlow: React.FC<Props> = ({
  ...
  useEffect(() => {
    ...
    renderer.setPixelRatio(baseDprRef.current);
    renderer.shadowMap.enabled = false;
    renderer.outputColorSpace = THREE.SRGBColorSpace;
    renderer.setClearColor(0x000000, 0);
    try { (renderer as any).preserveDrawingBuffer = true; } catch (e) {}
    ...
    canvas.classList.add('laser-canvas');
    ...
    // DPR autoscaling removed to avoid visual flashes when the renderer changes pixel ratio.
    ...
    const animate = () => {
      ...
      mouseSmooth.set(mouseSmooth.x + (mx - mouseSmooth.x) * alpha, mouseSmooth.y + (my - mouseSmooth.y) * alpha);
  ...
  useEffect(() => {
    ...
    uniforms.uCoreScale.value = coreScale;
